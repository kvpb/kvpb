...
class RegistrationsControllerTest < ActionDispatch::IntegrationTest
  ...
  test "create builds a regular, non-admin account while registration is open" do
    Setting.current.update!( registration_enabled: true )
    ...
    assert_not User.find_by!( username: "newbie" ).admin?
    assert cookies[ :session_id ]
  end

  test "create ignores an injected admin param" do
    Setting.current.update!( registration_enabled: true )

    post registration_path, params: { user: { username: "newbie", email_address: "newbie@example.com", password: "password123", password_confirmation: "password123", admin: true } }
