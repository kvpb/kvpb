...
class HonoreesControllerTest < ActionDispatch::IntegrationTest
  ...
  test "index redirects guests to root when the hall of fame is empty" do
    Honoree.published.destroy_all
  ...
  test "index still renders for a signed-in admin when the hall of fame is empty" do
    Honoree.published.destroy_all
    sign_in_as( users( :one ) )
