...
if admin_credentials.blank?
  raise <<~MESSAGE
    Missing `admin` credentials. Add them with:
...
user = User.find_or_initialize_by( email_address: admin_credentials[ :email_address ] )
user.username = admin_credentials[ :username ]
user.password = admin_credentials[ :password ] if user.new_record?
user.admin = true
user.save!
...
# through the (unlinked, admin-only) milestones controller.
...
# Neither is ever seeded in production.
unless Rails.env.production? || Article.exists?( headline: "Placeholder headline for layout testing" )
  Article.create!(
...
if !Rails.env.production? && print_sample_path.exist? && !Print.exists?( title: "Placeholder title for layout testing" )
