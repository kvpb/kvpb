...
export default class extends Controller {
  ...
  // clamping each axis to its own ±max independently, the way this used to work, bounds the drag to
  // a square: reachable further on the diagonal, toward a corner, than straight toward an edge — the
  // diagonal alone gets the full maxX and maxY at once, out to maxX√2 from center, while an edge
  // midpoint is capped at maxX or maxY alone. Clamping the vector's own length instead, once, bounds
  // it to a circle — the same maximum reach in every direction, corners included
  onPointerMove( event ) {
    if ( !this.dragging ) return
    const rect = this.element.getBoundingClientRect()
    const maxRadius = Math.min( rect.width, rect.height ) * this.maxDragFractionValue
  ...
  // Where this element is drawn is read back from the page, not worked out from its offset: the Cover Flow's
  // side covers lean in 3D under a shared perspective, so a step of so many pixels shows as a slightly larger
  // one on their near edge, and only what is drawn says how much room is really left. The frame is another
  // element on the page (boundsSelector, looked up among this one's siblings): the element may travel up and
  // down only while its top and bottom stay between the frame's own top and bottom, and where it meets one of
  // them it stops there and rebounds off it, with a fraction of its speed (bounce), rather than passing beyond it
  // to be cut off by whatever clips the stage. Sideways it is free. An element already taller than the frame
  // has no room to move at all, so it stays put vertically rather than be pushed back and forth across it
  keepInsideBounds( previousOffsetY ) {
    this.boundsElement ||= this.element.parentElement?.querySelector( this.boundsSelectorValue )
    if ( !this.boundsElement ) return
    const frame = this.boundsElement.getBoundingClientRect()
    ...
    // the drawn box is as much taller than the element's own height as its lean stretches it, and that is how
    // much further than the step the box travels: the nearest edge, which is the one that reaches the frame first
    // and the one the box's own top and bottom are made of, moves by the step times that stretch
    const stretch = Math.max( 1, box.height / this.element.offsetHeight )
    const roomUp = Math.max( box.top - frame.top, 0 ) / stretch
    const roomDown = Math.max( frame.bottom - box.bottom, 0 ) / stretch
    const step = this.offsetY - previousOffsetY
    if ( step < -roomUp ) {
      this.offsetY = previousOffsetY - roomUp
      if ( this.velocityY < 0 ) this.velocityY = -this.velocityY * this.bounceValue
    } else if ( step > roomDown ) {
      this.offsetY = previousOffsetY + roomDown
      if ( this.velocityY > 0 ) this.velocityY = -this.velocityY * this.bounceValue
    }
  }
  ...
  tick() {
    ...
    if ( this.hasBoundsSelectorValue ) this.keepInsideBounds( previousOffsetY )
