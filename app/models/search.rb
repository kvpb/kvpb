  ...
  private
    ...
    def sections
      ...
      if word_matches?( :read ) && Article.published.exists?
        ...
        found << Result.new( kind: "section", title: "journal", detail: "#{ count( Article.published.count, "article" ) } · latest: #{ excerpt( latest.headline ) }", path: routes.read_path, image: image_path( latest.cover_image ) )
      end
      ...
      if word_matches?( :see ) && Album.published.exists?
        ...
        found << Result.new( kind: "section", title: "gallery", detail: summary, path: routes.see_path, image: image_path( latest.cover ) )
      end
    ...
    def about
      matched = matching( Skill.where( category: Skill::DISPLAYED_CATEGORIES ).ordered, :name ).pluck( :name ) +
        matching( Milestone.visible.chronological, :title, :organization, :location, :description, :date_label ).pluck( :title )
      ...
      [ Result.new( kind: "section", title: "get to know & contact", detail: detail, path: routes.root_path ) ]
    end
