...
class Photo < ApplicationRecord
  ...
  # What a browser is shown of this photograph. A JPEG, PNG, GIF or WebP goes out exactly as it is;
  # anything else Active Storage can convert — an iPhone's HEIC above all, which no browser can display —
  # is shown as a JPEG made from it by ImageMagick, stood upright by its own orientation, the first time
  # anyone asks for it, and kept from then on like any other file. The original is never touched: it stays in
  # storage as it came, with its own name and its own bytes, and is what the photograph is always read from
  def web_image
