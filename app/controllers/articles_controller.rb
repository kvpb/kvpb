class ArticlesController < ApplicationController
  ...
  def index
    scope = admin? ? Article.all : Article.published.unscope( :order )
    @articles = paginate( scope.front_page_ordered )
  end

  def show
    @comments = @article.comments.visible.order( :created_at )
    @pending_comments = admin? ? @article.comments.pending.order( :created_at ) : Comment.none
    @comment = Comment.new
  end
  ...
  private
    ...
    def require_visible_article
      raise ActiveRecord::RecordNotFound if !@article.published? && !admin?
    end

    def redirect_if_section_empty
      redirect_to root_path if helpers.section_empty?( :journal ) && !admin?
    end
