class AlbumsController < ApplicationController
  ...
  def index
    scope = admin? ? Album.all.order( taken_until: :desc ) : Album.published
  ...
  private
    ...
    def require_visible_album
      raise ActiveRecord::RecordNotFound if !@album.published? && !admin?
    end

    def redirect_if_section_empty
      redirect_to root_path if helpers.section_empty?( :gallery ) && !admin?
    end
