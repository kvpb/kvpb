class HonoreesController < ApplicationController
  ...
  def index
    @honorees = admin? ? Honoree.all.order( :name ) : Honoree.published
  end
  ...
  private
    ...
    def require_visible_honoree
      raise ActiveRecord::RecordNotFound if !@honoree.published? && !admin?
    end

    def redirect_if_section_empty
      redirect_to root_path if helpers.section_empty?( :hall_of_fame ) && !admin?
    end
