module ApplicationHelper
  # an image_tag for a photograph, its corners squircled by squircle_clip_controller.js. Its own controller is added to
  # whatever data-controller the caller gave, not put in place of it
  def photo_tag( source, options = {} )
    data = ( options[ :data ] || {} ).dup
    data[ :controller ] = [ data[ :controller ], "squircle-clip" ].compact_blank.join( " " )
    image_tag( source, options.merge( data: data ) )
  end
  ...
  def current_section_suffix
    ...
    elsif %w[messages milestones skills photo_dwells photo_dwell_events settings].include?( controller_name )
